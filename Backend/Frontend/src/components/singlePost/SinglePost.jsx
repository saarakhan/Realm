...
const singlePost = () => {
  ...
  useEffect(() => {
    const getPost = async () => {
      const res = await axios.get(`${window.location.origin}/api/post/` + path);
      setPost(res.data);
  ...
  const handleDelete = async () => {
    try {
      await axios.delete(`${window.location.origin}/api/post/` + path, { data: { username: user.username } });
      setDel(true);
      setTimeout(() => {
  ...
  const handleUpdate = async () => {
    try {
      await axios.put(`${window.location.origin}/api/post` + path, { username: user.username, title, desc });
      setUpdate(true);
      setTimeout(() => {
